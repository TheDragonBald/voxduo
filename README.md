...
```powershell
just install-dev # инструменты разработки: ruff, mypy, pytest, PyInstaller + git-хуки
just --list      # все команды
just all         # хуки, лок, линтер, типы и тесты перед коммитом
...
`just install-dev` ставит их сам; отдельно — `just hooks`. На свежем клоне в
`.git/hooks` лежат только образцы `*.sample`, которые git не исполняет, — то есть
до этой команды хуков нет вовсе.

| Когда | Что делает |
| ----- | ---------- |
| перед коммитом | ruff проверяет только застейдженные `.py` — около полутора секунд |
| перед пушем | линтер, лок, типы и тесты целиком — около 11 секунд на тёплом кеше |

Пуш тега и удаление ветки (`git push origin :ветка`) эти проверки не гоняют
вовсе: лефтхук читает stdin git и видит, что реального кода в пуше нет.

**Хук ничего не чинит сам — он отказывает и говорит, что делать.** Форматирование
остаётся за вами: `just fmt`, затем `git add`. Так сделано осознанно: чтобы
вернуть спрятанные незастейдженные правки, хук должен не менять файлы под ними —
иначе на конфликте возврата lefthook 2.1.7–2.1.12 стирает их безвозвратно
([#1480](https://github.com/evilmartians/lefthook/issues/1480)). Проверено
прогоном на обеих версиях; подробности и таблица — в комментарии `lefthook.yml`.

Хуки **не являются гейтом**: `git commit --no-verify` и переменная `LEFTHOOK=0`
обходят их целиком, и это нормально — они покупают скорость обратной связи, а не
доверие. Единственная настоящая преграда для сломанного кода — обязательная
проверка `checks` в защите ветки `main`.

Первый пуш после `just clean` дороже примерно вшестеро: рецепт удаляет
`.mypy_cache`, и проверка типов считается заново.

**Если хук оборвался посреди работы** — упал, был убит, машина выключилась, — а в
дереве пропали незастейдженные правки: они не потеряны. На время прогона lefthook
прячет их и кладёт копию в двух местах:

```powershell
git stash list                  # запись «lefthook auto backup»
git apply --recount --unidiff-zero .git/info/lefthook-unstaged.patch
```

Обычного `Ctrl+C` это не касается — его lefthook перехватывает и возвращает
спрятанное сам.
