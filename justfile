...
# удалённый хук или переключённый core.hooksPath — пропускает молча.
#
...
#
# Каталог хуков спрашиваем у git, а не пишем «.git/hooks» литералом: в связанном
# worktree `.git` — файл со ссылкой, и литерал не нашёл бы там ничего никогда.
# `git rev-parse --git-path hooks` отдаёт `.git/hooks` в обычном репозитории и
# абсолютный путь к общему каталогу в worktree — проверено обоими случаями.
#
# Полноценной работы в worktree это всё равно не даёт, и обещать её не нужно:
# там нет своего `.venv`, поэтому первая строка рецепта падает раньше второй
# («Failed to spawn: lefthook»). Проект worktree не использует осознанно —
# решение записано в CLAUDE.md. Правка сделана ради честности проверки, а не
# ради поддержки сценария.
[doc("Проверить, что хуки стоят и не разошлись с lefthook.yml")]
check-hooks:
    {{ run }} lefthook check-install
    @$hd = (git rev-parse --git-path hooks); foreach ($h in 'pre-commit','pre-push') { if (-not (Test-Path "$hd/$h")) { Write-Host "Хук $h не установлен. Поставить: just hooks"; exit 1 } }
