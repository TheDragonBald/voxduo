...
# что должен чинить.
#
# [doc(...)] явно задаёт описание для just --list: just берёт последней
# строкой блока комментариев, а она здесь — часть прозы, не резюме.
[doc("Доставить инструменты разработки")]
install-dev:
...
# --- git-хуки ---
#
# lefthook.yml описывает pre-commit (ruff по застейдженным файлам — только
# проверяет, никогда не правит: почему именно так, с прогонами, написано там же)
# и pre-push (линтер, лок, типы, тесты; пропускается для пуша тега и удаления
# ветки).
# Хуки не гейт: --no-verify и LEFTHOOK=0 обходят их целиком, единственный
# настоящий гейт — обязательный джоб checks в защите main. Хуки только
# ускоряют обратную связь, до неё не подменяя.
...
[doc("Поставить git-хуки из lefthook.yml")]
hooks:
...
[doc("Проверить, что хуки стоят и не разошлись с lefthook.yml")]
check-hooks:
...
[doc("Проверить стиль и форматирование")]
lint:
...
[doc("Отформатировать и починить, что чинится автоматически")]
fmt:
...
[doc("Всё, что стоит прогнать перед коммитом")]
